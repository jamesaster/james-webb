...
**The pool's job.**
Keep Polars off Streamlit's rerun thread, not to parallelize the math. Each @supreme call still blocks on its own worker - sidebar, KPI cards, and charts still compute one after another. st.fragment(parallel=True) only parallelizes UI rendering. But even without it, Polars running directly on Streamlit's ScriptRunner was what caused the Windows access violation in the first place - the pool isn't an optimization, it's what makes calling Polars from Streamlit survivable at all.
...
**Serial ≠ quantity, but same table anyway.** Serialized items (electronics, devices) and regular quantity-based items share the same ledger, same table, same logic. The SQL enforces "max stock of 1 per serial" and "a product can't mix serial and non-serial history" as constraints, not app-level checks.
