...
def page_config(title: str = 'Raw Project', icon: str = '🐲'):
    with st.sidebar:
...
@st.fragment
def sideBar_filter_backup(df: pl.DataFrame, adv_list: list=[S.staff, S.cat]):
    """
    ### Sidebar filter subsystem implementing a One-Way Valve pattern using Fragments.
    ...
    m_n        = utils['this_month_name']
    p_m        = utils['prev_month_count']
    min_date   = start = utils['min_date']
    max_date   = end   = utils['max_date']
    last_ops   = {'d': 'Day', 'w': 'Week', 'mo': 'Month'}
    last_x_key = 'last_x_key'
    last_y_key = 'last_y_key'
    last_z_key = 'last_z_key'
    ...
    with st.sidebar:
        ...
        mode  = st.selectbox(
            label   = '**:blue[\\*] Select Period**',
            options = [f'{m_n} So Far', f'Last {p_m} days', 'Last X Y', 'All Time', 'Custom']
            )
        if mode == 'Custom':
            with st.expander('Default: Last 7 days', expanded=True, icon=':material/edit_calendar:'):
                start = st.date_input('Start',
        ...
        if mode == 'Last X Y':
            # Phải dùng form vì nếu click nhanh quá polars sập ở tầng Rust
            # with st.form(key='filter_form', border=True, width='stretch'):
                st.number_input(
                    label     = 'X: Time Value',
                    value     = 1,
                    min_value = 1,
                ...
                st.space()
                # st.form_submit_button(label='**Apply Filter**', width='stretch', icon=':material/filter_alt:')

    x = SS.get(last_x_key, 0)
    y = SS.get(last_y_key, 'mo')
    z = SS.get(last_z_key, [])
    xyz_Expr = []
    if 'Head' in z and y != 'd':
        xyz_Expr.append(DATE >= DTAIL.dt.offset_by(f'-{x}{y}').dt.truncate(f'1{y}'))
    ...
    df_dated    = df.filter(config[mode])
    prev_anchor = df_dated.select([
        (pl.col(S.date).min() - (pl.col(S.date).max() - pl.col(S.date).min() + timedelta(1))).alias('lower_bound'),
        pl.col(S.date).min().alias('upper_bound'),
        pl.col(S.date).max().alias('current_upper')
    ]).row(0, named=True)
    curr_bound  = prev_anchor.pop('current_upper')
    df_prev     = df.filter(pl.col(S.date).is_between(**prev_anchor, closed='left'))
    information = st.sidebar.empty()
    #endregion

    #region # Advanced
    adv_masks = [pl.lit(True)]
    adv_utils = df_dated.select([pl.col(c).drop_nulls().unique().implode() for c in adv_list]).row(0, named=True)
    for key, vals in adv_utils.items():
        adv_select_all = f'All {key.title()}'
        vals = [adv_select_all] + vals
        res  = st.sidebar.selectbox(
            label    = f'**:red[\\*] {key.title()}**',
            options  = sorted(vals, key=custom_sort),
            key      = f'adv_{key}',
            index    = 0
            )
        if res != adv_select_all:
            adv_masks.append(pl.col(key) == res)
    adv_Expr = pl.all_horizontal(adv_masks)
    df_final = df_dated.filter(adv_Expr)
    #endregion
    ...
    dfe_key  = 'date_filter_expression'
    afe_key  = 'advanced_filter_expression'
    dfe_str  = str(config[mode])
    afe_str  = str(adv_Expr)
    ...
    if df_final.is_empty():
        ...
        information.write(
            f':{badge}[:material/filter_arrow_right:]\u2000\
            :{badge}-badge[{prev_anchor['upper_bound'].strftime('%d-%m-%Y')}]\
            :{badge}[:material/chevron_forward:]\
            :{badge}-badge[{curr_bound.strftime('%d-%m-%Y')}]'
            )
        if dfe_key not in SS:
            SS[dfe_key]  = dfe_str
            SS[afe_key]  = afe_str
        elif SS[dfe_key] != dfe_str:
            SS[dfe_key] = dfe_str
            st.rerun(scope='app')
        elif SS[afe_key] != afe_str:
            SS[afe_key] = afe_str
            st.rerun(scope='app')
    #endregion
    ...
    return {
        ...
        'df_dated': df_dated,
        'df_final': df_final,
        'period'  : pl.date_range(prev_anchor['upper_bound'], curr_bound).alias(S.date),
    }
